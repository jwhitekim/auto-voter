...
from .stats import parse_ran_at_kst, KST
...
def format_stats_message(history: list[dict]) -> str:
    ...
    today_records = [h for h in valid if (dt := parse_ran_at_kst(h["ran_at"])) and dt.date() == today]
    week_records = [h for h in valid if (dt := parse_ran_at_kst(h["ran_at"])) and dt.date() >= week_start]
